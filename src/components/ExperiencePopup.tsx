...
export default function ExperiencePopup({ experience, onClose }: ExperiencePopupProps) {
  ...
  const headerBg = bgColorMap[experience.color] ?? 'bg-black';
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`${headerBg} text-white p-6 sm:p-8`}>
          <span className="b2 px-3 py-1 bg-white/20 inline-block mb-3">{experience.type}</span>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {experience.tech_stack.split(',').map((tech) => (
                <span key={tech} className="b2 px-3 py-1 bg-black/5 border-2 border-black">
                  {tech.trim()}
                </span>
              ))}
            </div>
